...
// Ported from the old Arduino Nano code.
// Sends a 10µs trigger pulse and measures the echo return time.
// NOTE: pulseIn() blocks for up to ~30ms worst case — acceptable in loop().
...
// GET /status
// Flask calls this every ~2s to check connectivity and machine state.
// Response shape must match what esp32.py expects:
//   { state, glass_present, uptime, last_pour_ml }
...
// POST /dispense
// Flask sends: { amount_ml: int, request_id: string }
// Respond 200 to acknowledge; non-200 signals failure to Flask.
